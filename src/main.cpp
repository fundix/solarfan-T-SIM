...
void modem_on()
{
    // Make sure the UART is up so we can talk to the modem
    // (SerialAT.begin() is already called in gsmSetup() before we enter here.)

    Serial.println("Checking if modem already ON...");
    if (modem.testAT(500))
    { // quick probe: send "AT" and wait 500 ms for "OK"
        Serial.println("Modem already powered, skip PWRKEY pulse");
        return; // nothing to do
    }

    // Otherwise toggle the PWRKEY pin to switch the modem on
    Serial.println("Toggling PWRKEY to power modem on");
    pinMode(PWR_PIN, OUTPUT);
    digitalWrite(PWR_PIN, LOW); // level before pulse
    delay(10);
    digitalWrite(PWR_PIN, HIGH); // active‑HIGH pulse (≈1 s)
    delay(1100);
    digitalWrite(PWR_PIN, LOW); // back to idle

    Serial.println("Waiting until modem is ready…");
    delay(6000); // give SIM7000 time to boot (NET light will start blinking)
}
...
void gsmConnect(void)
{
    ...
    // --- Hologram GSM / GPRS configuration sequence ---
    modem.sendAT("+CFUN=0");                        // 1) disable RF while we tweak settings
    modem.sendAT("+CNMP=13");                       // 2) GSM only
    modem.sendAT("+CGDCONT=1,\"IP\",\"hologram\""); // 3) set APN for Hologram
    vTaskDelay(200 / portTICK_PERIOD_MS);           // give modem a breather
    modem.sendAT("+CFUN=1");                        // 4) re‑enable RF (no hard reset)
    vTaskDelay(10000 / portTICK_PERIOD_MS);         // allow time to attach
...
void mqttSetup()
{
    modem.sendAT("AT+CMQTTSTART");
    modem.waitResponse();
    modem.sendAT("AT+CMQTTACCQ=0,\"solarfan_client\",0");
    modem.waitResponse();
}

bool mqttConnect()
{
    String cmd = String("AT+CMQTTCONNECT=0,\"tcp://") + broker + ":" + mqttPort + "\",60,1";
